...
##Añadimos el respositorio de PostgreSQL's repository. ## Llamamos a la 9,3. Si cambiamos de version es posible ##que tengamos que modificar el Dockerfile
...
RUN apt-get update && apt-get -y --force-yes -q install python-software-properties software-properties-common \
&& apt-get -y --force-yes -q install postgresql-9.3 postgresql-client-9.3 postgresql-contrib-9.3
##Nos cambiamos al usuario postgres, que se ha creado ##al instalar postgreSQL
USER postgres
##Creamos un usuario denominado “pguser” con password ##”secret” y creamos una base de datos llamada “pgdb”
...
RUN chmod +x /usr/local/bin/entrypoint.sh
   ...
   USER postgres
##Creamos 3 variables para crear el usuario, ##la password y la base de datos
ENV PASS=secret
ENV BBDD=pgdb
ENV USER=pguser
